    ...
    if text.strip():
        ...
        with st.spinner("Очеловечиваем текст..."):
            ...
            response = client.chat.completions.create(
                model="meta-llama/llama-3.1-8b-instruct:free",  # новая модель
                messages=[
                    {
                        "role": "system",
                        "content": "Ты переписываешь текст в естественный человеческий стиль."
                    },
                    {
                        "role": "user",
                        "content": f"""
Перепиши текст так, чтобы он звучал естественно и по-человечески.

ТРЕБОВАНИЯ:
- используй разные по длине предложения
- избегай шаблонных и «идеальных» формулировок
- добавь лёгкую разговорность
- сделай текст менее формальным
- убери признаки AI текста

ВАЖНО:
- сохрани смысл
- не сокращай сильно
- добавь небольшие естественные несовершенства
